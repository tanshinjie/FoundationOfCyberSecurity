...
def genRoundKeys(key):
    ...
    for i in range(1,FULLROUND+1):
        ...
        # salt
        key ^= i << 15
    key_registry[FULLROUND+1] = key >> 16
    return key_registry
...
def pLayer(state,mode):
    ...
    for i in range(64):
        if mode == 'e':
            new_state += ((state >> i) & 0x1) << pmt[i]
        if mode == 'd':
            new_state += ((state >> i) & 0x1) << pmt_inv[i]
    return new_state
...
if __name__ == "__main__":
    ...
    round2 = present_round(round11, key1)
